	...
	$.fn.extend({
		scratchOn: function(mask,options) {
			...
			function init(){
				...
				if(opts.brush){
					icom.imageLoad(opts.brush,function(src){
						var image=new Image();
						image.src=src;
						image.width=40*windowScale;
						image.height=40*windowScale;
						$brush = new ibrush({canvas:$canvas[0], image:image});
					});
				}//end if
				else{
